...
public class RecommendationsGrpcService extends RecommendationsControllerGrpc.RecommendationsControllerImplBase {
    ...
    public void getRecommendationsForUser(UserPredictionsRequestProto request,
        ...
        List<UserInteraction> recent = userInteractionRepository
        ...
        List<UserInteraction> allInteractions = userInteractionRepository.findByIdUserId(userId);
        Map<Long, Double> userWeights = allInteractions.stream()
                .collect(Collectors.toMap(i -> i.getId().getEventId(), UserInteraction::getWeight, (a, b) -> a));
    ...
    public void getSimilarEvents(SimilarEventsRequestProto request,
        ...
        Set<Long> interacted = userInteractionRepository.findByIdUserId(userId).stream()
                .map(i -> i.getId().getEventId())
                .collect(Collectors.toSet());

        List<RecommendedEventProto> results = eventSimilarityRepository.findAllByEvent(eventId).stream()
                .map(sim -> {
    ...
    public void getInteractionsCount(InteractionsCountRequestProto request,
        ...
        Map<Long, Double> sums = userInteractionRepository.sumWeightsByEventIds(eventIds).stream()
                .collect(Collectors.toMap(UserInteractionRepository.EventWeightSumProjection::getEventId,
                        UserInteractionRepository.EventWeightSumProjection::getWeightSum));
    ...
    private Map<Long, Double> collectCandidates(Set<Long> seedEvents, Set<Long> interactedEvents) {
        List<EventSimilarity> similarities = eventSimilarityRepository.findAllByEventsIn(new ArrayList<>(seedEvents));
    ...
    private double predictScore(Long candidateId, Map<Long, Double> userWeights) {
        ...
        List<EventSimilarity> sims = eventSimilarityRepository.findAllForEventAndOthers(candidateId, interactedIds);
